                    ...
                    cellSpaceMemberList.map((cellSpaceMember, index)=>{
                        ...
                        surfaceMemberList.map((surfaceMember,surfaceMemberIndex)=>{
                            ...
                            posList.map((pos,posIndex)=>{
                                ...
                                let splitedVector = pos['_'] != undefined ? pos['_'].split(' ') : pos.split(' ');
                                ...
                                if (surfaceMemberIndex == 0){
                                    ...
                                    if (pos['_'] != undefined) {
                                        ...
                                        pos = cellSpaceCoordinateArray[index][matchingPos[splitedVector]][0] + ' '+cellSpaceCoordinateArray[index][matchingPos[splitedVector]][1]+' ' + pos.split(' ')[2];
                                    }
                                }else {
                    ...
                    spaceLayerMemberList.map((spaceLayerMember)=>{
                        ...
                        if (stateMemberList != undefined) {
                            stateMemberList.map((stateMember)=>{
                                ...
                                let statePos = Point['gml:pos'] != undefined ? Point['gml:pos'] : Point['pos'];
                                
                                statePos.map((pos)=>{
                                    if (pos['_'] != undefined) {
                                        pos['_'] = stateCoordinateArray[stateIndex][0] + ' ' + stateCoordinateArray[stateIndex][1] + ' ' + pos['_'].split(' ')[2];
                                    }else{
                                        pos = stateCoordinateArray[stateIndex][0] + ' ' + stateCoordinateArray[stateIndex][1] + ' ' + pos.split(' ')[2];
                                    }
                                })
                                    ...
                                    TransitionList.map((Transition)=>{
                                        ...
                                        LineStringPosList.map((pos,posIndex)=>{
                                            ...
                                            if (pos['_'] != undefined) {
                                                ...
                                                pos = transitionCoordinateArray[transitionIndex][posIndex][0] + ' ' + transitionCoordinateArray[transitionIndex][posIndex][1] + ' ' + pos.split(' ')[2];
                                            }
        ...
        return new Promise((resolve,reject)=>{
            ...
            reader.onload = () => {
                ...
                parser.parseStringPromise(this.readed_indoorGML).then((result)=>{
                    ...
                    let primalSpaceFeatures;
                    let multiLayeredGraph;


                    let PrimalSpaceFeatures;
                    let cellSpaceMemberList;

                    
                    for (let [key, value] of Object.entries(result)) {
                        IndoorFeatures = value;
                    }
                    
                    for (let [key, value] of Object.entries(IndoorFeatures)) {
                        if (key.split(':').reverse()[0] == 'primalSpaceFeatures') primalSpaceFeatures = value;
                        if (key.split(':').reverse()[0] == 'multiLayeredGraph') multiLayeredGraph = value;
                    }
                    
                    for (let [key, value] of Object.entries(primalSpaceFeatures)) {
                        PrimalSpaceFeatures = value;
                        for (let [key, value] of Object.entries(PrimalSpaceFeatures)) {
                            cellSpaceMemberList = value;
                        }
                    }
                    
                    cellSpaceMemberList.map((cellspaceMember)=>{
                        
                        let CellSpace;
                        let cellSpaceGeometry;
                        let Geometry3D;
                        let Solid;
                        let exterior;
                        let Shell;
                        let surfaceMemberList;

                        for (let [key, value] of Object.entries(cellspaceMember)) {
                            CellSpace = value;
                        }

                        for (let [key, value] of Object.entries(CellSpace)) {
                            cellSpaceGeometry = value;
                        }

                        for (let [key, value] of Object.entries(cellSpaceGeometry)) {
                            Geometry3D = value;
                        }

                        for (let [key, value] of Object.entries(Geometry3D)) {
                            Solid = value;
                        }

                        for (let [key, value] of Object.entries(Solid)) {
                            exterior = value;
                        }

                        for (let [key, value] of Object.entries(exterior)) {
                            Shell = value;
                        }

                        for (let [key, value] of Object.entries(Shell)) {
                            surfaceMemberList = value;
                        }
                        ...
                        surfaceMemberList.map((surfaceMember,surfaceMemberIndex)=>{
                            if (surfaceMemberIndex == 0){
                                ...
                                let LinearRing;
                                let posList;
                                for (let [key, value] of Object.entries(surfaceMember)) {
                                    Polygon = value;
                                }
                                for (let [key, value] of Object.entries(Polygon)) {
                                    exterior = value;
                                }
                                for (let [key, value] of Object.entries(exterior)) {
                                    LinearRing = value;
                                }
                                for (let [key, value] of Object.entries(LinearRing)) {
                                    posList = value;
                                }
                                console.log(posList);
                    ...
                    console.log(cellSpaceMemberList);
